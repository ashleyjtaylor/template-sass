...
- **`const` / `let` declarations** — blank line below, unless the next statement is also a `const` / `let`. Group related declarations together, then break before the next phase. If consts are just setting primitives, group them. If const are being set by async requests or functons, break them up.
...
- **`utils.ts`** - any utility or helper function that is specific to the module goes here.
...
- No dumping helper files in to `src/lib` - place logical module helpers in a more understandable place. Only put it n `src/lib` if it is a generic helper or used it multiple modules.

## After applying changes

Run `pnpm lint && pnpm format && pnpm typecheck`. Fix all errors before reporting done.
